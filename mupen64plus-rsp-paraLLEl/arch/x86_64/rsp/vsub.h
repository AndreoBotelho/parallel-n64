...
static inline __m128i rsp_vsub(__m128i vs, __m128i vt,
  ...
  unsat_diff = simde_mm_sub_epi16(vt, carry);
  sat_diff = simde_mm_subs_epi16(vt, carry);

  *acc_lo = simde_mm_sub_epi16(vs, unsat_diff);
  vd = simde_mm_subs_epi16(vs, sat_diff);
  ...
  overflow = simde_mm_cmpgt_epi16(sat_diff, unsat_diff);
  return simde_mm_adds_epi16(vd, overflow);
}
